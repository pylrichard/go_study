...
// Config is the runner app config struct
type Config struct {
...
func (p *program) run() {
	...
	if p.Stderr != "" {
		f, err := os.OpenFile(p.Stderr, os.O_CREATE | os.O_APPEND | os.O_WRONLY, 777)
		if err != nil {
	...
	if p.Stdout != "" {
		f, err := os.OpenFile(p.Stdout, os.O_CREATE | os.O_APPEND | os.O_WRONLY, 777)
		if err != nil {
...
func getConfigPath() (string, error) {
	...
	log.Printf("dir: %v, execName: %v, ext: %v, name: %v", dir, execName, ext, name)
...
func Build() {
	svcFlag := flag.String("service", "", "Control system service")
	flag.Parse()
